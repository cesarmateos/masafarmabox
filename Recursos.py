...
def leerConfig():
    ...
    global feed
    global backfeed
    global imprimirRadio
    global imprimirDetalle
    ...
    imprimirRadio = int(configParser.get('Ticket','radio'))
    imprimirDetalle = int(configParser.get('Ticket','detalle'))
...
def leerItem(grupo,item):
    configParser = ConfigParser()   
    rutaConfig = rutaArchivo('Recursos/config.ini')

    configParser.read(rutaConfig)
    
    return configParser.get(grupo,item)
